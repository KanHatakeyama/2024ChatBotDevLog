...
class Preprocessor:
    ...
    def add_bos(self, sample):
        ...
        """
        sample[self.result_key] = self.tokenizer.bos_token + \
            sample[self.result_key]
        return sample
    """
    ...
    def preprocess_openai_messages(self, sample, messages, role='role', content='content', add_system_message=True, ignore_original_system_message=True, add_eos=False):
        ...
        turns = sum([1 if 'user' in m[role] or 'human' in m[role]
...
def load_datasets(data_config_list, template_config, cache_dir, tokenizer, split):
    ...
    for data_config in data_config_list:
        if split in data_config['split']:
            dataset = load_dataset(
                data_config['name'], split=data_config['split'][split], cache_dir=cache_dir)
            # dataset = dataset.map(lambda sample: preprocessor.apply_preprocessing(sample, data_config['preprocess']))
            processed_samples = [preprocessor.apply_preprocessing(
                sample, data_config['preprocess']) for sample in dataset]
            dataset = Dataset.from_dict(
                {key: [sample[key] for sample in processed_samples] for key in processed_samples[0].keys()})
    ...
    print('=' * 30 + '\n' + f'{split} data info:\n' + '\n'.join(
        [f' {k}: {v} samples' for k, v in dataset_info.items()]) + '\n' + '=' * 30)
...
def main() -> None:
    parser = HfArgumentParser(
        (TrainingArguments, SFTTrainingArguments, DataArgments))
    ...
    train_dataset = load_datasets(
        data_config_list, template_config, dataset_cache_dir, tokenizer, 'train')
    if training_args.do_eval:
        print("do eval")
        eval_dataset = load_datasets(
            data_config_list, template_config, dataset_cache_dir, tokenizer, 'eval')
    ...
    is_local_rank_0 = (torch.distributed.get_rank() % get_accelerator(
    ).device_count() == 0) if torch.distributed.is_initialized() else True
    if is_local_rank_0:
        import socket
        import wandb
        ...
        save_scripts = sft_training_args.save_scripts.split(
